...
export function RowActions({ row, table }) {
  ...
  // POST /approvals/approve-payment  { id }
  // PHP: sets papproval=1, payamentapproved_by, payamentapproved_on
  //      closes notification "Approve Payment", echoes "Reload : Payment Approved"
  const handleApprove = useCallback(async () => {
    const id = row.original.id;
    setLoading(true);
    try {
      await axios.post("/approvals/approve-payment", { id });
