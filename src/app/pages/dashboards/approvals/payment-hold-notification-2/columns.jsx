...
export const columns = [
  ...
  // PHP: $n[] = $row['inwardid'];  order by inwardentry.id desc
  ...
  // PHP: $n[] = str_replace(",", "<br/>", group_concat(bookingrefno)) from crfinstrument{tablesuffix}
  ...
  // PHP: $n[] = str_replace(",", "<br/>", group_concat(lrn)) from crfinstrument{tablesuffix}
  ...
  // PHP: $n[] = changedateformatespecito($row['requestedon'], "Y-m-d H:i:s", "d/m/Y H:i:s");
  ...
  // PHP: $n[] = concat(firstname,' ',lastname) from admin where id=added_by
  ...
  // PHP: if (in_array(404, $permissions)) → show "Request For Non Payment Report" button
  columnHelper.display({
    id: "actions",
    header: "Action",
    cell: RowActions,
