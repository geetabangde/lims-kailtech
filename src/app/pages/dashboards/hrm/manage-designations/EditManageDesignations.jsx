...
export default function EditModes() {
  ...
  useEffect(() => {
    const fetchModes = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`/hrm/get-designation-byid/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`hrm/update-designation/${id}`, form);
      const result = response.data;

      if (result.status === "true") {
        toast.success(result.message || "Manage designations updated successfully", {
          duration: 1000,
          icon: "✅",
        });
        ...
        toast.error(result.message || "Failed to update Manage designations ❌");
      }
    } catch (err) {
  ...
  return (
    <Page title="Edit Manage Designation">
      <div className="p-6">
        {/* Header + Back Button */}
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-gray-800 dark:text-white">
            Edit Manage Designation
          </h2>
          <Button
            variant="outline"
            className="text-white bg-blue-600 hover:bg-blue-700"
            onClick={() => navigate("/dashboards/hrm/manage-designations")}
          >
            Back to List
          </Button>
        </div>
