...
const confirmMessages = {
  pending: {
    description:
      "Are you sure you want to delete this attendance policy? Once deleted, it cannot be restored.",
  },
  success: {
...
export function RowActions({ row, table }) {
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      await axios.delete(`/hrm/delete-attendance-policy/${id}`);
  ...
  return (
    <>
      <div className="flex items-center justify-center gap-2">
        <Link
          to={`/dashboards/hrm/view-attendance-policies/edit/${row.original.id}`}
