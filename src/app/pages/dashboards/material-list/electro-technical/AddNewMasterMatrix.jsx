...
const MasterChecklistForm = () => {
      ...
      try {
        ...
        if (instrumentsData && Array.isArray(instrumentsData) && instrumentsData.length > 0) {
          const instrumentOptions = instrumentsData.map(item => ({
            value: item.id,
            label: item.name || item.instrument_name || item.description || `Instrument ${item.id}`
          }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post('/material/add-site-checklist', payload);
      ...
      // Navigate back to the checklist view after a short delay
      setTimeout(() => {
        const redirectUrl = `/dashboards/material-list/electro-technical/view-checklist/${id || 96}`;
      ...
      toast.error(error.response?.data?.message || 'Failed to add master checklist. Please try again.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-6xl mx-auto bg-white rounded-lg shadow-sm">
        ...
        <div className="flex justify-between items-center p-4 border-b border-gray-200">
          ...
          <Button 
            ...
            onClick={() => navigate(`/dashboards/material-list/electro-technical/view-checklist/${id || 96}`)}
          >
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center">
                ...
                <div className="flex-1">
                  <Select
                    ...
                    onChange={(option) => handleSelectChange('discipline', option)}
                    options={disciplines}
                    ...
                    styles={{
                      control: (provided) => ({
                        ...provided,
                        borderColor: errors.discipline ? '#ef4444' : provided.borderColor,
                        '&:hover': {
                          borderColor: errors.discipline ? '#ef4444' : provided.borderColor,
                        }
                      })
              ...
              <div className="flex items-center">
                ...
                <div className="flex-1">
                  <Select
                    ...
                    onChange={(option) => handleSelectChange('equipmentForVerification', option)}
                    options={instruments}
              ...
              <div className="flex items-start">
                ...
                <div className="flex-1">
                  ...
                  />
                  {errors.generalCheck && (
                    <p className="text-red-500 text-sm mt-1">{errors.generalCheck}</p>
                  )}
                </div>
              </div>
                ...
                <div className="flex-1">
                  <Select
                    ...
                    styles={{
                      control: (provided) => ({
                        ...provided,
                        borderColor: errors.unit ? '#ef4444' : provided.borderColor,
                        '&:hover': {
                          borderColor: errors.unit ? '#ef4444' : provided.borderColor,
                        }
                      })
              ...
              <div className="flex items-center">
                ...
                <div className="flex-1">
                  <Input 
                    ...
                    className={`w-full px-3 py-2 border rounded focus:outline-none focus:ring-1 focus:ring-cyan-500 focus:border-cyan-500 ${
                      errors.acceptanceLimit ? 'border-red-500' : 'border-gray-300'
                    }`}
                    placeholder="Enter acceptance limit"
                  />
                  {errors.acceptanceLimit && (
                    <p className="text-red-500 text-sm mt-1">{errors.acceptanceLimit}</p>
                  )}
                </div>
              </div>
            </div>
