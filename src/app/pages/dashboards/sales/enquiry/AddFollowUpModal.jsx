...
export default function AddFollowUpModal({ show, id, mode, onClose, onSuccess, title }) {
  const [formData, setFormData] = useState({
    ...
    remark: "",
    followup: "contact", // "contact", "interested"
    next_follow_up_date: "",
  ...
  const displayTitle = title || (isCloseMode ? "Close Enquiry" : "Add Follow-Up Interaction");
  ...
  const handleSubmit = async (e) => {
    ...
    if (isCloseMode) {
      payload = {
        ...
        what: "Close" // Added based on PHP logic seen earlier
      };
    } else {
      payload = {
        enquiryid: Number(id),
    ...
    try {
      const res = await axios.post("/sales/add-follow-up", payload);
            ...
            {isCloseMode ? (
              <div className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <Textarea
                    name="reason"
                    placeholder="Specify why this lead is being finalized..."
                    rows={4}
                    required
