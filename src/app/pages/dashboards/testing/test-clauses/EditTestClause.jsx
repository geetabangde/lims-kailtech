  ...
  useEffect(() => {
    const fetchModes = async () => {
      try {
        ...
        const response = await axios.get(`/testing/get-clauses-byid/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.status === "true") {
        ...
        toast.error(result.message || "Failed to update test clause ❌");
      }
    } catch (err) {
    ...
    <Page title="Edit Standard">
      <div className="p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <Input
              label="Test Clause Name"
              value={mode.modeName}
              onChange={(e) => handleInputChange('modeName', e.target.value)}
              placeholder="Enter test clause name"
            />  
            {errors.modeName && (
